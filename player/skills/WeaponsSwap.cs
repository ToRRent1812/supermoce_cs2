...
namespace jRandomSkills
{
    public class WeaponsSwap : ISkill
    {
        ...
        private static readonly string[] PrimaryWeapons = [ "weapon_mp9", "weapon_mac10", "weapon_bizon", "weapon_mp7", "weapon_ump45",
        "weapon_p90", "weapon_mp5sd", "weapon_famas", "weapon_galilar", "weapon_m4a4", "weapon_m4a1_silencer", "weapon_ak47",
        ...
        private static void UpdateHUD(CCSPlayerController player, PlayerSkillInfo skillInfo, bool showInfo)
        {
            ...
            if (showInfo)
                ...
                remainingLine = cooldown != 0 ? $"<font class='fontSize-m' color='#FFFFFF'>{Localization.GetTranslation("hud_info", $"<font color='#FF0000'>{cooldown}</font>")}</font> <br>" : $"<font class='fontSize-s' class='fontWeight-Bold' color='#FFFFFF'>{skillData.Description}</font>";
        ...
        public static void UseSkill(CCSPlayerController player)
        {
            ...
            if (SkillPlayerInfo.TryGetValue(player.SteamID, out var skillInfo))
            {
                ...
                if (skillInfo.CanUse)
                {
                    ...
                    if (enemyWeapon != null && enemyWeapon.FirstOrDefault(w => PrimaryWeapons.Contains(w)) == null)
                    {
                    ...
                    if (enemyWeapon != null && playerWeapon != null)
                    {
                        ...
                        Server.NextFrame(() =>
                        {
                            enemy.GiveNamedItem("weapon_knife");
                            GiveWeapons(player, enemyWeapon, playerWeapon.Contains("weapon_c4"));
                            if (enemyWeapon.Contains("weapon_c4")) enemy.GiveNamedItem("weapon_c4");
                            if (enemy.Team == CsTeam.Terrorist) enemy.GiveNamedItem("weapon_glock");
                            else enemy.GiveNamedItem("weapon_usp_silencer");
                        });
                        //GiveWeapons(enemy, playerWeapon, enemyWeapon.Contains("weapon_c4"), true);
                    }

                    enemy.PrintToChat($" {ChatColors.DarkRed}{player.PlayerName}{ChatColors.LightRed} ukradł Ci sprzęt.");
        ...
        {
            if (weapons == null || player == null || !player.IsValid || player.Health <= 0) return;
