    ...
    public class Shade : ISkill
    {
        ...
        public static void LoadSkill()
        {
            ...
            Instance.RegisterEventHandler<EventPlayerHurt>((@event, info) =>
            {
                ...
                if (attacker == null || victim == null || !Instance.IsPlayerValid(attacker) || !Instance.IsPlayerValid(victim)) return HookResult.Continue;

                var victimInfo = Instance.SkillPlayer.FirstOrDefault(p => p.SteamID == victim?.SteamID);
                var attackerInfo = Instance.SkillPlayer.FirstOrDefault(p => p.SteamID == attacker?.SteamID);

                if (attackerInfo?.Skill == skillName && Instance.Random.NextDouble() <= 0.50f)
        ...
        public class SkillConfig(Skills skill = skillName, bool active = true, string color = "#9d9d9d", CsTeam onlyTeam = CsTeam.None, bool needsTeammates = false, float teleportDistance = 100f) : Config.DefaultSkillInfo(skill, active, color, onlyTeam, needsTeammates)
        {
